...
class OAuthFilterGithub(OAuthFilterBase):
    ...
    def __init__(self, iface):
        super(OAuthFilterGithub, self).__init__(iface)

    def login(self):
        # Step 1. Get a request token from Github.
        callback_url = self.get_current_url()
        state = hashlib.md5(str(time.time())).hexdigest()
        url = "%s?callback_uri=%s&client_id=%s&state=%s" % (authenticate_url, quote(callback_url), OAUTH2_CLIENT_ID, state)
        # Just store the key
        self.request_storage[state] = True
    ...
    def authenticated(self, request_token, verifier_token):
        ...
        """
        This is what you'll get back from Github. Note that it includes the
    ...
    def get_access_token(self):
        ...
        request_token = params.get('CODE', None)
        verifier_token = params.get('STATE', None)
