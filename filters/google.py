...
request_token_url = GOOGLE_TOKEN_URI
access_token_url = GOOGLE_TOKEN_URI
authenticate_url = GOOGLE_AUTH_URI


class OAuthFilterGoogle(OAuthFilterBase):
    ...
    def __init__(self, iface):
        super(OAuthFilterGoogle, self).__init__(iface)

    def login(self):
        try:
            request_token = self.request_storage['request_token']
        except KeyError:
            # Step 1. Get a request token from Google.
    ...
    def authenticated(self, request_token, verifier_token):
        ...
        if resp['status'] != '200':
            ...
            raise OAuthException("Invalid response from OAuth endpoint.")
        """
        This is what you'll get back from Google. Note that it includes the
    ...
    def get_access_token(self):
        ...
        if access_token is not None:
            if self.token_storage.get(access_token):
                ...
                # TODO: re-validate the token against Google endpoint
                raise OAuthException('access_token in request is NOT verified!')
